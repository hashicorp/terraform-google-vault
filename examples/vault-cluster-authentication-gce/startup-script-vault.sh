...
# The Vault Role name, Project ID and the allowed zones and labels are being passed by terraform
# This example will allow GCP resources in a given zone and with given labels to authenticate
# Read more at: https://www.vaultproject.io/api/auth/gcp/index.html#gce-only-parameters
/opt/vault/bin/vault write \
  auth/gcp/role/${example_role_name}\
  project_id="${project_id}" \
  type="gce" \
  policies="example-policy" \
  bound_zones="${vault_auth_allowed_zones}" \
  bound_labels="${vault_auth_allowed_labels}"
