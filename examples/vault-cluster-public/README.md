...
fronted by a Regional External Load Balancer using the [vault-cluster](/modules/vault-cluster) and [vault-lb-fr](
/modules/vault-lb-fr) modules. For an example of a private Vault cluster that is accessible only from inside the Google
Cloud VPC, see [vault-cluster-private](/examples/vault-cluster-private). **Deploying Vault in a publicly accessible way
...
We accomplish this by using the [run-nginx](/modules/run-nginx) module to run the web server. 
